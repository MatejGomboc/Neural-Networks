...
CController::CController(HWND hwndMain, CParams* pParams)
	//create population of sweepers
	: m_vecSweepers(pParams->iNumSweepers, pParams->iNumInputs, pParams->iNumNeurons, pParams->iNumOutputs, pParams->stMutationParams)
{
	m_bFastRender = false;
	m_iTicks = 0;
	m_iGenerations = 0;
	...
	for (unsigned i = 0; i < pParams->iNumMines; ++i)
	{
		m_vecMines.push_back(SVector2D(RandFloat() * rect.right, RandFloat() * rect.bottom));
	}
...
//	method.
//-------------------------------------------------------------------
void CController::WorldTransform(vector<SPoint> &VBuffer, SVector2D vPos, CParams pParams)
{
	//create the world transformation matrix
	C2DMatrix matTransform;

	//scale
	matTransform.Scale(pParams.dMineScale, pParams.dMineScale);
...
//  graph showing best and average score
//------------------------------------------------------------------------
void CController::PlotStats(HWND hwndMain, HDC surface)
{
	string s = "Worst Fitness:       " + ftos(m_vecSweepers.WorstFitness);
	TextOut(surface, 5, 20, (LPCWSTR)s.c_str(), s.size());

	s = "Average Fitness: " + ftos(m_vecSweepers.AverageFitness);
	TextOut(surface, 5, 40, (LPCWSTR)s.c_str(), s.size());
	...
	//render the graph
	float HSlice = (float)rect.right / (float)(m_iGenerations + 1);
	float VSlice = (float)rect.bottom / (float)((m_vecSweepers.BestScore() + 1 ) * 2);
	...
	MoveToEx(surface, 0, rect.bottom, NULL);

	for (int i=0; i < m_vecBestScore.size(); ++i)
	{
		LineTo(surface, x, rect.bottom - VSlice * m_vecBestScore[i]);
		x += HSlice;
	}
	...
	for (int i = 0; i < m_vecAvScore.size(); ++i)
	{
		LineTo(surface, (int)x, (int)(rect.bottom - VSlice * m_vecAvScore[i]));
		x += HSlice;
	}
